...
export const SupabaseAuthModal: React.FC<SupabaseAuthModalProps> = ({
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    // Validação de emergência para master admin (caso o backend esteja sem internet)
    const isMasterAdminEmail = cleanEmail === 'tarira.ecossistema@gmail.com' ||
                               cleanEmail === 'tariraecossystem@gmail.com' ||
                               cleanEmail === 'admin@tarira.co.mz';
    const isMasterAdminPass = password === 'Sahombe13' || 
                              password.toLowerCase() === 'sahombe13' || 
                              password === 'admin123' || 
                              password.toLowerCase() === 'tarira2025' || 
                              password === 'tarira.2025';
      ...
      if (isSupabaseConfigured && supabase) {
        try {
          ...
          if (!error && data?.user) {
            ...
            // Guardar o token criptográfico Cyber-Shield para chamadas protegidas
            try {
              const tokenRes = await fetch('/api/auth/login', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
  ...
  const handleAdminSignIn = async (e: React.FormEvent) => {
    ...
    const isAdminEmailMatch = cleanAdminEmail === 'tarira.ecossistema@gmail.com' ||
                              ...
                              cleanAdminEmail === 'admin@tarira.co.mz';
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto p-3 sm:p-6 py-6 sm:py-10 bg-slate-950/90 backdrop-blur-md animate-fade-in flex justify-center items-start sm:items-center">
      <div className="relative w-full max-w-lg rounded-3xl bg-white border border-slate-200 shadow-2xl p-6 sm:p-8 max-h-[88vh] overflow-y-auto my-auto">
        ...
        {mode === 'signup' && (
          <div className="mb-4 p-3 rounded-xl bg-slate-50 border border-blue-200 text-[#172554] text-xs flex items-start gap-2">
            ...
            <div className="leading-relaxed">
              ...
              <span>1. Escolha o tipo de perfil pretendido (Lar Particular, Empresa ou Condomínio) → 2. Preencha os campos de identificação e contacto → 3. Clique em Concluir Registo.</span>
            </div>
          </div>
        )}
            ...
            {role === 'lar' && (
              <div className="space-y-3.5 p-4 rounded-2xl bg-white border border-blue-200 shadow-sm">
                <div className="flex items-center gap-2 pb-1.5 border-b border-slate-100 text-[#172554]">
                  ...
                  <span className="text-xs font-bold font-mono uppercase tracking-wider text-[#172554]">Dados da Residência Particular</span>
                </div>
                ...
                {role === 'profissional' && (
                  <div className="p-3.5 rounded-xl bg-blue-50 border border-blue-200">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2.5 rounded-xl bg-blue-50 border border-blue-300 text-[#172554] text-xs focus:outline-none focus:border-blue-400 font-mono font-bold"
                    />
                    <span className="text-[9.5px] text-blue-600/80 block mt-1">
